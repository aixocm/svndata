...
class Spider(CrawlSpider):
    ...
    DBFans = db["Fans"]
    DBFollows = db["Follows"]

    all = set()
    now = set()

    for item in DBInformation.find({}):
        now.add(item["_id"])

    for item in DBFans.find({}):
        all.add(item["_id"])
        for i in range(1, len(item)):
            all.add(item[str(i)])

    for item in DBFollows.find({}):
        all.add(item["_id"])
        for i in range(1, len(item)):
            all.add(item[str(i)])
    ...
    def parse0(self, response):
        ...
        if text0:
            ...
            yield Request(url=url_information1, meta={"item": informationItems, "ID": response.meta["ID"]}, callback=self.parse1)
    ...
    def parse3(self, response):
        ...
        for elem in text2:
            ...
            if elem:
                ...
                if self.DBInformation.find({"_id": ID}).count() == 0:
                    ID = str(ID)
                    url_tweets = "http://weibo.cn/%s/profile?filter=1&page=1" % ID
                    url_information0 = "http://weibo.cn/attgroup/opening?uid=%s" % ID
                    yield Request(url=url_information0, meta={"ID": ID}, callback=self.parse0)  # 去爬个人信息
